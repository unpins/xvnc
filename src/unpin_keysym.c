/* Self-contained keysym helpers for the in-process xkbcomp: XStringToKeysym +
 * XConvertCase + XKeysymToString, vendored from libX11 (StrKeysym.c /
 * KeysymStr.c table searches + KeyBind.c case conversion) with the XKeysymDB/
 * Xrm machinery stripped out. No display, no /nix/store refs, no libX11 link.
 * ks_tables.h is generated at build time by makekeys from xorgproto's
 * keysymdef.h, exactly as libX11's own build does.
 */
#include <stddef.h>
#include <string.h>
#include <stdlib.h>
...
#define NEEDKTABLE              /* gate ks_tables.h K-table (name->keysym) in */
#define NEEDVTABLE              /* and the V-table (keysym->name) for XKeysymToString */
#include "ks_tables.h"
...
typedef unsigned int Signature;   /* matches libX11's Xresinternal.h */

/* libX11's XStringToKeysym, minus the optional XKeysymDB lookup (Xrm-based,
 * implementation-defined, almost never present) -- the static table plus the
 * U+XXXX / 0x hex / XF86_ fallbacks cover everything xkeyboard-config needs. */
KeySym
XStringToKeysym(_Xconst char *s)
{
    ...
    /* Inconsistency between headers and the old XKeysymDB: some XF86* syms had
     * a separating underscore there. As a last ditch effort, try without. */
...
/* libX11's XKeysymToString, minus the registered-keysym Xrm database path. The
 * static V-table covers every named keysym; unrecognized Unicode keysyms fall
 * back to the "U+XXXX" spelling in a static buffer (xkbcomp is single-threaded
 * and only uses this for diagnostics / .xkb text output). */
char *
XKeysymToString(KeySym ks)
{
    ...
    if (ks >= 0x01000100 && ks <= 0x0110ffff) {
        ...
        int i = (val & 0xff0000) ? 10 : 6;
        i--;
        ubuf[i--] = '\0';
        for (; i; i--) {
            unsigned char d = val & 0xf;
            val >>= 4;
            ubuf[i] = (d < 10) ? ('0' + d) : ('A' + d - 10);
        }
        ubuf[i] = 'U';
        return ubuf;
    }
